...
/*
 * convert 3 bytes of raw value from sensor to pressure in mmHg
 */
...
/*
 * read 3 bytes of raw value data from sensor over i2c
 */
float readData() {
...
/* 
 * record the pressure value, store in an array for future calculation
 */
void recordPressure() {
  ...
  // increase to 200 mmHg, dont record data before deflating
  ...
  // start recording when deflating, sample every 0.2 second
  for (int i = 0; i < 1000; ++i) {
    current_pressure = readData();
    // stop recording when reaches 30 mmHg
    ...
    // print current pressure on lcd
    snprintf(display_buf[0],60,"pressure: %.2f", current_pressure);
    lcd.DisplayStringAt(0, LINE(8), (uint8_t *)display_buf[0], LEFT_MODE);
    printf("pressure: %.2f\n", current_pressure);
    // sample every 0.2 second
    wait_us(200000);
    // record the slope of pressure curve
...
/*
 * calculate systolic pressure from the recorded array
 */
float getSystolicPressure() {
  int heartBeatCount = 0;
  // any pressure reading with positive slope with at least 1 reading with positive
  // slope within the following 12 readings is considered the systolic pressure
  ...
  // return 0 when can't find systolic pressure
  return 0.0;
}

/*
 * find where mean pressure is in the recorded array
 */
int getMapIdx() {
  float maxSlope = 0.0;
  int maxSlopeIdx = 0;
  // find the biggest ripple, it's the map
  for (int i = 0; pressureData[i] > 80; ++i) {
    if (slope[i] > 0) {
...
/*
 * calculate diastolic pressure from systolic pressure and mean pressure
 */
float getDiastolicPressure(float systolicPressure, float MapPressure) {
  return MapPressure - systolicPressure + MapPressure;
}

/*
 * Calculate heart beat per minute
 */
int getBpm() {
  int totalHeartBeat = 0;
  // find the number of heart beats in 100 samples around mean pressure (20 seconds)
  for (int i = mapIdx -50; i < mapIdx +50; ++i) {
    if (slope[i] > 0 && slope[i+1] < 0) {
      totalHeartBeat++;
    }
  }
  // multiply total number of heart beats in 20s by 3, it's bpm
...
int main() {
  ...
  while(1) {
    ...
    float diastolicPressure = getDiastolicPressure(systolicPressure, pressureData[mapIdx]);
    // get bpm
    int bpm = getBpm();
    ...
    printf("====================== end ======================\n");
    // wait 20 seconds if you want to do another round of mesurement
    wait_us(20000000);
    lcd.Clear(LCD_COLOR_WHITE);
  }
}